...
export async function GET() {
  try {
    ...
    if (usersError) {
      if (isMissingTableError(usersError.message) || isTransientBackendError(usersError.message)) {
        return NextResponse.json({ totalUsers: 0, rates: emptyRates() });
      }
    ...
    if (ideasError) {
      if (isMissingTableError(ideasError.message) || isTransientBackendError(ideasError.message)) {
    ...
    if (messagesError) {
      if (isMissingTableError(messagesError.message) || isTransientBackendError(messagesError.message)) {
    ...
    const message = error instanceof Error ? error.message : "Unexpected error while loading achievement stats.";
